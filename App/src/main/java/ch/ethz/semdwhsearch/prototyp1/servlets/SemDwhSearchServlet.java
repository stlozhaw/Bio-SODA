	...
	private Data initApplicationAndLoadData(HttpServletRequest req, Data data) {
		...
		if (autoInitFile.exists()) {
			try {
				...
				try {
					dataDirPath = Paths.get(getClass().getClassLoader().getResource(dataDirPath).toURI()).toFile().getAbsolutePath();
				} catch(Exception e) {
				...
				try {
					configDirPath = Paths.get(getClass().getClassLoader().getResource(configDirPath).toURI()).toFile().getAbsolutePath();
				} catch(Exception e) {
